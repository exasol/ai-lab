...
def test_make_ami_public(default_asset_id):
    ...
    mock_cast(aws.list_amis).return_value = [mock_ami]
    mock_cast(aws.get_ami).return_value = Ami(mock_public_ami_data)
    run_make_ami_public(aws, default_asset_id)
    mock_cast(aws.modify_image_launch_permission).assert_called_once_with(
        mock_ami.id, {
            'Add': [ { 'Group': 'all', }, ],
        }
    )
...
def test_make_ami_public_not_changing_public_ami(default_asset_id):
    ...
    mock_cast(aws.list_amis).return_value = [mock_ami]
    run_make_ami_public(aws, default_asset_id)
    mock_cast(aws.get_ami).assert_not_called()
    mock_cast(aws.modify_image_launch_permission).assert_not_called()
...
def test_make_ami_public_not_changed_raises_exception(default_asset_id):
    ...
    mock_ami = get_ami_image_mock_data("AVAILABLE")
    mock_cast(aws.list_amis).return_value = [mock_ami]
    mock_cast(aws.get_ami).return_value = mock_ami
