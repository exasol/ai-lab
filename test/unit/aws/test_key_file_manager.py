...
def test_external_keys(tmp_path, default_asset_id):
    """"
    Test that external key files will be used if given and the file won't
    be deleted when calling close()
    """
    aws_access_mock = MagicMock()
    tmp_file = tmp_path / "tst.pem"
    with open(tmp_file, "w") as f:
        f.write("secret")
    with KeyFileManagerContextManager(KeyFileManager(
            aws_access_mock,
            "test_key",
            str(tmp_file),
...
def test_generated_key(default_asset_id):
    """
    Test that generated key files will be created on AWS and removed when
    calling close().
    """
    aws_access_mock = MagicMock()
    aws_access_mock.create_new_ec2_key_pair.return_value = "secret_abc"
    key_name = ""
    with KeyFileManagerContextManager(
            KeyFileManager(
                aws_access_mock, None, None, default_asset_id.tag_value,
            )) as km:
        assert len(km.key_name) > 0
        aws_access_mock.create_new_ec2_key_pair.assert_called_once_with(
    ...
    # Now call close. After that we expect that the key has been removed from
    # AWS and the temporary file was removed.
