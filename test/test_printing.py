...
from exasol.ds.sandbox.lib.asset_printing.print_assets import print_with_printer, AssetTypes
...
def test_printing_ami(default_asset_id, printing_mocks):
    ...
    print_with_printer(aws_mock, None, (AssetTypes.AMI.value,), "*", printing_factory)
...
def test_printing_snapshot(default_asset_id, printing_mocks):
    ...
    print_with_printer(aws_mock, None, (AssetTypes.SNAPSHOT.value,), "*", printing_factory)
...
def test_print_export_image_tasks(default_asset_id, printing_mocks, progress, status, status_message, mock_data):
    ...
    print_with_printer(aws_mock, None, (AssetTypes.EXPORT_IMAGE_TASK.value,), "*", printing_factory)
...
def test_print_cloudformation_stack(default_asset_id, printing_mocks, filter_value, expected_found_cloudformation):
    ...
    print_with_printer(aws_mock, asset_id, (AssetTypes.CLOUDFORMATION.value,), filter_value, printing_factory)
...
def test_print_ec2_keypairs(default_asset_id, printing_mocks):
    ...
    print_with_printer(aws_mock, None, (AssetTypes.EC2_KEY_PAIR.value,), "*", printing_factory)

    assert table_printer_mock.add_column.call_count == 4

    table_printer_mock.add_row.assert_called_once_with('key-123', 'ec2-key-test-key',
                                                       "2022-08-16, 15:30",  default_asset_id.tag_value)
